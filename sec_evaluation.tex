...
%\henrique{In the paper~\cite{z-ranking}, the author mentioned that the lower the S(R) the better. I did not have time to read deeper and understand if that was particurlar for his study or a general characteristic of this metric. Do you know abou this? If indeed lower is better, then it should be mentioned.}
%\kleidi{as can also be seen from the formula, sums the amount of FP you have to inspect for each actionable alert, and lower is indeed better}
...
%\kleidi{Release/Revision testing because to the reasons explained in the previous section}
...
%\henrique{How many times exactly did you run the experiments? (yes this is important). Moreover, how did you select the metric when you run multiple times? Or did you compute the average, or median?}
%\kleidi{When compared with random ranking, the evaluation is run 1000 times (when the set is small) or 10 (when using the whole set, can increase it), as can also be seen on the result tables. Note, that only the random score is evaluated multiple times, as the test score is static (because the test set is static 80/20 sequential split). Each time the random score is compared with the test set score (lower is better), and counted how many times the test set score is better.}
%\henrique{Was that mentioned somewhere in the thesis? If so you can place a reference after 'run multiple experiments' with the section. If not it is important to explicitly describe what you just explained.}
...
%\henrique{I am a little confused here. The section is about actionable alerts but in the first sentence you state the alerts are ranked for the predcition actionable or not. Then how are you measuring actionable alerts?}
%\kleidi{The classifier is trained to recognize if an alert is actionable or not. Internally it has a probability score for each class (ac or not ac), and that probability is used for ranking the alerts. The classical scores like precision/recall, are calculated on the class prediction of the classifier, the ranking scores are calculated on the ranked order based on the probability.}
%\henrique{Ok, now I understood. Do you think you can change the sentence to make it more clear and explicit? For example instead of using the expression 'actionable or not' using 'wether it is actionable'. If I got confused, a committee member might as well.}
...
\henrique{Are you sure about these results? If the random sample has a f-score of 95\%, I do not see much gain with other techniques with 96\%. Moreover, a two percent different among techniques may be statiscally insignificant.}
\kleidi{The idea/goal of comparing cleaning/balancing techniques, which are anyway important to consider, was a product of the results of my previous test set, which was smaller and had also more errors inside. In that previous test set, the difference was much more noticeable, 10+ percent improvement. In this case the difference is smaller, and indeed maybe not statistically significant.} 
\henrique{Remember your explaination well, you may face a question about these results during the defense.}
\kleidi{Also, I fear I might have some overfitting in here, though it is difficult to say with certainty, as I have high scores both on the kfold validation as well as the test set. The explanation is that there is overfitting, but both the train and test sets have very similar distributions.}
...
%\henrique{If you concluded something here, it should appear in your conclusion.}
...
\henrique{By using Cross Validation or Released/Revision? Moreover, why this split gives better results?}
\kleidi{Always revision split, the reason explained in the description of evaluation methods. I do not know the exact reason.}
\henrique{Explicitly mention that you are using Released/Revision, since you can do it in a few words. There could be underlying reasons on why this split gives better results. Be prepared for a question about this on your defense.}
\kleidi{I have said it in the begging of the evaluation section, it is thus the same split for all methods. I highlighted it to be clear.}


The algorithm ranks alerts types based on two input weight parameters ($\alpha,\beta$). By performing a simple for-loop that iterates over different input variable values (with \textit{a} ranging from 0.1 to 0.9 and \textit{b=1-a}) and inspecting the results, the following values were chosen: $\alpha=0.7, 
\beta=0.3$. 
%\henrique{How did you conduct this experimental evaluation to discover these values? Trying values at random is not 'experimental evaluation'. Experimental evaluation implies a method or at least some structure.}
%\kleidi{a for loop, with \textit{a} ranging from 0.1 to 0.9 and \textit{b=1-a}}
%\henrique{Ok, then please add this explaination to the text.}
...
\kleidi{Because I did not know that beforehand, I found out when experimenting. Also because the precise weight of the alert type does not matter, only their order based on the weights}
\henrique{Maybe there is a better way to write the last sentence to avoid this impression. Instead of writing 'the values did not make much difference', try something on the lines of 'slight variations on the results' and also include your explanation on the order based on the weights being more significant/important.}
...
%\kleidi{they were on the previous sections, added to the statement anyway.}
%\henrique{It is good to add values like this to make it easier and quicker to review. Even if you mentioned before.}
%Also almost all of them disappear with time, and can thus be considered actionable. 
...
%There is in this case a big overlap in the output of both the Actionable Alerts and Bug-Related Lines method, and since the first performs quite well, a combination of both in the actual form, is redundant (that does not mean that the method of BRLs in itself is redundant, because it works differently than Actionable Alerts). 
...
%That will be one of the inputs to our ensemble approach.

%\henrique{Do you understand you just "shot yourself in the foot" here. If they are redundant, why did you do it? How can you justify your research? Maybe you should rewrite and tone down the statement.}
%\kleidi{\textbf{Obviously I did not know that beforehand} when implementing the method. When analyzing the collected bug related alerts, it turned out that they belonged in 99\% of the cases to the set of closed alerts. 
%Also, the value of this method, its that it is quite simple, just calculating alert weights. It is nice to see if such a simple method can provide decent results.}
%\henrique{Nothing is obvious unless you explicit write it. If you write an explanation like you just gave fitting in the context of the above paragraph, it would show that you understand what is happening and the value of the results even though they are redundant.}
%\kleidi{Explained it differently.}
...
%\henrique{And what technique is this library using? Bayesian network?}
%\kleidi{The problem type as previously said, is a multi-objective optimization problem, nothing to do with bayesian networks. It uses genetic algorithms to find the optimal solution, based on the target scores.}
...
%\henrique{How can you be sure that fine tunning can achieve better results? Do you have evidence to back that up? How much better?}
%\kleidi{I cannot be sure, but other score functions can be used, other constraints... so it should be at least explored if someone else is interested in implementing something similar. It is not the scope of this thesis to find the best possible ensemble configuration, but to show that it is possible to combine them and get 'better' results. Changed 'can be achieved' to 'may be achieved'.}
%\henrique{Exactly, if you cannot be sure then it is best to avoid such strong statements/claims. You could also use 'it is worth exploring in future research if ...' but the 'may be achieved' is ok.}
