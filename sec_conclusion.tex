...
Excluding the Feedback Rank method, which did not scale to the size of our test project (could be because of implementation problems), the other methods all provide a noticeable improvement over random ranking of alerts (\cref{evaluation:results}). \henrique{(...) ranging from XX\% to XX\% improvement over the random...}
 The Actionable Alerts approach, provides great results in detecting actionable alerts (\cref{result:actalerts}) \henrique{wth a precision of XX\%,recall of XX\%, and...}
